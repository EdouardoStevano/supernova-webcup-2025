export const exportFarewellPageWithEmbedding = async (farewellPage) => {
  ...
  const exportDoc = document.implementation.createHTMLDocument("Farewell Page");
    ...
    try {
      ...
      if (originalSrc.startsWith("data:")) {
        ...
        try {
          ...
          console.warn("Could not embed GIF, using original URL:", embedError);
          gifSrc = originalSrc;
        }
      }
  ...
  const elementsPromises = farewellPage.elements.map(async (element) => {
    ...
    switch (element.type) {
      case "text":
        ...
        elementDiv.style.lineHeight = element.style?.lineHeight || "1.5";
      ...
      case "gif":
        ...
        const gifElement = { ...element, src: element.content || element.src };

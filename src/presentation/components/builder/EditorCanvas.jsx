...
const EditorCanvas = () => {
  ...
  return (
    ...
    >
      <div
        className={`h-[80vh] w-full relative overflow-hidden rounded-2xl shadow-xl border-4 border-white/20 ${farewellPage.background}`}
      >
        <div className="noisy-bg w-full h-full">
          {farewellPage.elements.map((element) => (
            <BuildingBlock key={element.id} element={element} />
          ))}

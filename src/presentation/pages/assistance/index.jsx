...
const Assistance = () => {
  ...
  const [message, setMessage] = useState("Je suis vraiment là pour toi, prête à t’aider à chaque étape. N’hésite surtout pas, tu peux compter sur moi. Tu n’es pas seule.");
  ...
  const startSpeechRecognition = () => {
    ...
    recognition.onerror = (event) => {
        ...
        if (event.error === 'network') {
            console.error('Erreur de reconnaissance vocale : vérifier la connexion Internet et les permissions micro.');
        }
        if (event.error === 'not-allowed') {
            console.error('Permission microphone refusée. Veuillez autoriser l’accès au microphone.');
  ...
  const sendToTTS = async (text) => {
    try {
      ...
      if (!blob.type.startsWith('audio/')) throw new Error('Type audio invalide');
